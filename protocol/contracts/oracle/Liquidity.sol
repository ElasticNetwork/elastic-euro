...
contract Liquidity is PoolGetters {
    ...
    function addLiquidity(uint256 euroAmount) internal returns (uint256, uint256) {
        (address euro, address usdc) = (address(euro()), usdc());
        (uint reserveA, uint reserveB) = getReserves(euro, usdc);

        uint256 usdcAmount = (reserveA == 0 && reserveB == 0) ?
             euroAmount :
             UniswapV2Library.quote(euroAmount, reserveA, reserveB);

        address pair = address(univ2());
        IERC20(euro).transfer(pair, euroAmount);
