...
contract Pool is PoolSetters, Liquidity {
    ...
    constructor(address euro, address univ2) public {
        _state.provider.dao = IDAO(msg.sender);
        _state.provider.euro = IEuro(euro);
    ...
    function _deposit(uint256 value) external onlyFrozen(msg.sender) notPaused {
    ...
    function _withdraw(uint256 value) external onlyFrozen(msg.sender) {
    ...
    function _claim(uint256 value) external onlyFrozen(msg.sender) {
    ...
    function _bond(uint256 value) external notPaused {
        unfreeze(msg.sender);
    ...
    function _unbond(uint256 value) external {
        unfreeze(msg.sender);
    ...
    function _provide(uint256 value) external onlyFrozen(msg.sender) notPaused {
        Require.that(
            totalBonded() > 0,
            FILE,
    ...
    function _emergencyWithdraw(address token, uint256 value) external onlyDao {
        IERC20(token).transfer(address(dao()), value);
    }

    function _emergencyPause() external onlyDao {
        pause();
    }
