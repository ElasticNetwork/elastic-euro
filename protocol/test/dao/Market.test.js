//TODO(laireht): improve logic of coupon redeem tests
  ...
  describe('redeemCoupons', function () {
    ...
    describe('after redeemable', function () {
      ...
      describe('on single call', function () {
        ...
        it('updates user balances', async function () {
          expect(await this.dollar.balanceOf(userAddress)).to.be.bignumber.equal(new BN(970223));
          expect(await this.market.balanceOfCoupons(userAddress, 1)).to.be.bignumber.equal(new BN(premium(1000000, 100000, 100000)));
        });

        it('updates dao balances', async function () {
          expect(await this.dollar.balanceOf(this.market.address)).to.be.bignumber.equal(new BN(29763));
        ...
        it('emits CouponRedemption event', async function () {
          ...
          expect(event.args.epoch).to.be.bignumber.equal(new BN(1));
          expect(event.args.couponAmount).to.be.bignumber.equal(new BN(70251));
        });
      });
      ...
      describe('multiple calls', function () {
        ...
        it('updates user balances', async function () {
          expect(await this.dollar.balanceOf(userAddress)).to.be.bignumber.equal(new BN(956212));
          expect(await this.market.balanceOfCoupons(userAddress, 1)).to.be.bignumber.equal(new BN(20000 + premium(1000000, 100000, 100000)));
        });

        it('updates dao balances', async function () {
          expect(await this.dollar.balanceOf(this.market.address)).to.be.bignumber.equal(new BN(43777));
        ...
        it('emits CouponRedemption event', async function () {
          ...
          expect(event.args.epoch).to.be.bignumber.equal(new BN(1));
          expect(event.args.couponAmount).to.be.bignumber.equal(new BN(35153));
        });
      });
    });
