//TODO(laireht): fix logic of treasury tests
...
describe('Regulator', function () {
  ...
  beforeEach(async function () {
    ...
    this.eurOracle = await MockSettableOracle.new({from: ownerAddress, gas: 8000000})
    this.regulator = await MockRegulator.new(this.oracle.address, this.eurOracle.address, poolAddress, {from: ownerAddress, gas: 8000000});
  ...
  describe('after bootstrapped', function () {
    ...
    describe('up regulation', function () {
      describe('above limit', function () {
        ...
        describe('on step', function () {
          beforeEach(async function () {
            await this.oracle.set(102, 100, true);
            await this.eurOracle.set(100, 100, true); // set it approx 2% below uniswap  price to trigger a 2% supply increase
            this.expectedReward = 20000;
          ...
          it('emits SupplyIncrease event', async function () {
            ...
            expect(event.args.currentPriceInUSD).to.be.bignumber.equal(new BN(102).mul(new BN(10).pow(new BN(16))));
      ...
      describe('(2) - only to bonded', function () {
        ...
        describe('on step', function () {
          beforeEach(async function () {
            ...
            await this.eurOracle.set(100, 100, true); // set it approx 1% below uniswap price to trigger a 1% supply increase
          ...
          it('emits SupplyIncrease event', async function () {
            ...
            expect(event.args.epoch).to.be.bignumber.equal(new BN(7));
            expect(event.args.currentPriceInUSD).to.be.bignumber.equal(new BN(101).mul(new BN(10).pow(new BN(16))));
            expect(event.args.newRedeemable).to.be.bignumber.equal(new BN(0));
            expect(event.args.lessDebt).to.be.bignumber.equal(new BN(0));
            expect(event.args.newBonded).to.be.bignumber.equal(new BN(this.expectedReward));
          });
      ...
      describe('(1) - refresh redeemable at specified ratio', function () {
        ...
        describe('on step', function () {
          beforeEach(async function () {
            await this.oracle.set(101, 100, true);
            await this.eurOracle.set(100, 100, true);
    ...
    describe('(3) - above limit but below coupon limit', function () {
        ...
        describe('on step', function () {
          ...
          it('emits SupplyIncrease event', async function () {
            ...
            expect(event.args.epoch).to.be.bignumber.equal(new BN(7));
            expect(event.args.currentPriceInUSD).to.be.bignumber.equal(new BN(105).mul(new BN(10).pow(new BN(16))));
            expect(event.args.newRedeemable).to.be.bignumber.equal(new BN(this.expectedRewardCoupons));
            expect(event.args.lessDebt).to.be.bignumber.equal(new BN(2000));
            expect(event.args.newBonded).to.be.bignumber.equal(new BN(this.expectedRewardLP + this.expectedRewardDAO));
          });
    ...
    describe('down regulation', function () {
      describe('under limit', function () {
        ...
        describe('on step', function () {
          beforeEach(async function () {
            await this.oracle.set(97, 100, true);
            await this.eurOracle.set(100, 100, true); // set it above oracle price to avoid triggering a mint
          ...
          it('emits SupplyDecrease event', async function () {
            ...
            expect(event.args.currentPriceInUSD).to.be.bignumber.equal(new BN(97).mul(new BN(10).pow(new BN(16))));
      ...
      describe('without debt', function () {
        ...
        describe('on step', function () {
          ...
          it('emits SupplyDecrease event', async function () {
            ...
            expect(event.args.epoch).to.be.bignumber.equal(new BN(7));
            expect(event.args.currentPriceInUSD).to.be.bignumber.equal(new BN(99).mul(new BN(10).pow(new BN(16))));
            expect(event.args.newDebt).to.be.bignumber.equal(new BN(this.expectedDebt));
          });
        });
      });
      ...
      describe('with debt some capped', function () {
        ...
        describe('on step', function () {
          beforeEach(async function () {
            await this.oracle.set(105, 100, true);
            await this.eurOracle.set(106, 100, true);
    ...
    describe('neutral regulation', function () {
      ...
      describe('on step', function () {
        beforeEach(async function () {
          await this.oracle.set(105, 100, true);
          await this.eurOracle.set(105, 100, true)
          this.result = await this.regulator.stepE();
          this.txHash = this.result.tx;
        });
    ...
    describe('not valid', function () {
      ...
      describe('on step', function () {
        beforeEach(async function () {
          ...
          await this.eurOracle.set(100, 100, false);
          this.result = await this.regulator.stepE();
          this.txHash = this.result.tx;
        });
