...
describe('Pool', function () {
  ...
  describe('frozen', function () {
    ...
    describe('when deposit', function () {
      beforeEach(async function () {
        ...
        this.result = await this.bonding._deposit(1000, {from: userAddress});
    ...
    describe('when withdraw', function () {
      describe('simple', function () {
        beforeEach(async function () {
          await this.univ2.faucet(userAddress, 1000);
          await this.univ2.approve(this.pool.address, 1000, {from: userAddress});
          await this.bonding._deposit(1000, {from: userAddress});

          this.result = await this.bonding._withdraw(1000, {from: userAddress});
          this.txHash = this.result.tx;
        });
      ...
      describe('too much', function () {
        beforeEach(async function () {
          ...
          await this.bonding._deposit(10000, {from: userAddress1});
        });

        it('reverts', async function () {
          await expectRevert(this.bonding._withdraw(2000, {from: userAddress}), "insufficient staged balance");
        });
      });
    });
    ...
    describe('when claim', function () {
      beforeEach(async function () {
        ...
        await this.univ2.approve(this.pool.address, 1000, {from: userAddress});
        await this.bonding._deposit(1000, {from: userAddress});
        await this.bonding._bond(1000, {from: userAddress});
        await this.dao.set((await this.dao.epoch()) + 1);
        await this.dollar.mint(this.pool.address, 1000);
        await this.bonding._unbond(1000, {from: userAddress});
        await this.dao.set((await this.dao.epoch()) + 1);
      });

      describe('simple', function () {
        beforeEach(async function () {
          this.result = await this.bonding._claim(1000, {from: userAddress});
          this.txHash = this.result.tx;
        });
      ...
      describe('too much', function () {
        ...
        it('reverts', async function () {
          await expectRevert(this.bonding._claim(2000, {from: userAddress}), "insufficient claimable balance");
        });
      });
    });
      ...
      describe('no reward', function () {
        describe('simple', function () {
          beforeEach(async function () {
            ...
            this.result = await this.bonding._bond(1000, {from: userAddress});
            this.txHash = this.result.tx;
          });
        ...
        describe('partial', function () {
          beforeEach(async function () {
            await this.univ2.faucet(userAddress, 1000);
            await this.univ2.approve(this.pool.address, 1000, {from: userAddress});
            await this.bonding._deposit(800, {from: userAddress});

            this.result = await this.bonding._bond(500, {from: userAddress});
            this.txHash = this.result.tx;
          });
        ...
        describe('multiple', function () {
          beforeEach(async function () {
            ...
            await this.bonding._deposit(1000, {from: userAddress1});

            await this.univ2.faucet(userAddress2, 1000);
            await this.univ2.approve(this.pool.address, 1000, {from: userAddress2});
            await this.bonding._deposit(1000, {from: userAddress2});

            await this.bonding._bond(600, {from: userAddress1});
            await this.bonding._bond(400, {from: userAddress2});
    ...
    describe('when unbond', function () {
      describe('without reward', function () {
        ...
        describe('simple', function () {
          beforeEach(async function () {
            this.result = await this.bonding._unbond(new BN(1000), {from: userAddress});
            this.txHash = this.result.tx;
          });
        ...
        describe('partial', function () {
          beforeEach(async function () {
            this.result = await this.bonding._unbond(new BN(800), {from: userAddress});
            this.txHash = this.result.tx;
          });
      ...
      describe('with reward', function () {
        ...
        describe('potential subtraction underflow', function () {
          beforeEach(async function () {
            ...
            await this.bonding._bond(600, {from: userAddress1});
            await this.bonding._bond(500, {from: userAddress2});

            await incrementEpoch(this.dao);
            await this.dollar.mint(this.pool.address, 1000);

            await this.bonding._unbond(new BN(1000), {from: userAddress});
            await this.bonding._bond(new BN(1000), {from: userAddress});
            await this.bonding._unbond(new BN(600), {from: userAddress});

            this.result = await this.bonding._unbond(new BN(200), {from: userAddress});
            this.txHash = this.result.tx;
          });
    ...
    describe('when provide', function () {
      ...
      describe('not enough rewards', function () {
        it('reverts', async function () {
          await expectRevert(this.bonding._provide(2000, {from: userAddress}), "Pool: insufficient rewarded balance");
        });
      });
      ...
      describe('simple', function () {
        ...
        beforeEach(async function () {
          ...
          this.result = await this.bonding._provide(1000, {from: userAddress});
          this.txHash = this.result.tx;
        });
      ...
      describe('complex', function () {
        ...
        beforeEach(async function () {
          ...
          await this.univ2.approve(this.pool.address, 1000, {from: userAddress1});
          await this.bonding._deposit(1000, {from: userAddress1});
          await this.bonding._bond(1000, {from: userAddress1});
  ...
  describe('fluid', function () {
    beforeEach(async function () {
      ...
      await this.bonding._bond(500, {from: userAddress});
    });
    ...
    describe('when deposit', function () {
      it('reverts', async function () {
        await expectRevert(this.bonding._deposit(1000, {from: userAddress}), "Pool: Not frozen");
      });
    });

    describe('when withdraw', function () {
      it('reverts', async function () {
        await expectRevert(this.bonding._withdraw(1000, {from: userAddress}), "Pool: Not frozen");
      });
    });

    describe('when claim', function () {
      it('reverts', async function () {
        await expectRevert(this.bonding._claim(1000, {from: userAddress}), "Pool: Not frozen");
      });
    });

    describe('when provide', function () {
      it('reverts', async function () {
        await expectRevert(this.bonding._provide(1000, {from: userAddress}), "Pool: Not frozen");
      });
    });
    ...
    describe('when unbond', function () {
      beforeEach(async function () {
        this.result = await this.bonding._unbond(new BN(500), {from: userAddress});
        this.txHash = this.result.tx;
      });
  ...
  describe('when pause', function () {
    beforeEach(async function () {
      ...
      await this.dao.set((await this.dao.epoch()) + 1);
      await this.dollar.mint(this.pool.address, 1000);
      await this.bonding._unbond(500, {from: userAddress});
      await this.dao.set((await this.dao.epoch()) + 1);
    });
    ...
    describe('as dao', function () {
      beforeEach(async function () {
        ...
        await this.bonding._emergencyPause({from: mockDao});
        await this.pool.set(this.dao.address, this.dollar.address, this.univ2.address);
      });
      ...
      it('reverts on deposit', async function () {
        await expectRevert(this.bonding._deposit(2000, {from: userAddress}), "Paused");
      });

      it('reverts on bond', async function () {
        await expectRevert(this.bonding._bond(2000, {from: userAddress}), "Paused");
      });

      it('reverts on provide', async function () {
        await expectRevert(this.bonding._provide(2000, {from: userAddress}), "Paused");
      });

      describe('withdraw', function () {
        beforeEach(async function () {
          await this.bonding._withdraw(200, {from: userAddress})
        });
      ...
      describe('unbond', function () {
        beforeEach(async function () {
          await this.bonding._unbond(200, {from: userAddress})
        });
      ...
      describe('claim', function () {
        beforeEach(async function () {
          await this.bonding._claim(200, {from: userAddress})
        });
    ...
    describe('as not dao', function () {
      it('reverts', async function () {
        await expectRevert(this.bonding._emergencyPause({from: userAddress}), "Not dao");
      });
    });
  });
  ...
  describe('when emergency withdraw', function () {
    ...
    describe('as dao', function () {
      beforeEach(async function () {
        await this.pool.set(mockDao, this.dollar.address, this.univ2.address);
        await this.bonding._emergencyWithdraw(this.univ2.address, 1000, {from: mockDao});
        await this.bonding._emergencyWithdraw(this.dollar.address, 1000, {from: mockDao});
      });
    ...
    describe('as not dao', function () {
      it('reverts', async function () {
        await expectRevert(this.bonding._emergencyWithdraw(this.univ2.address, 1000, {from: userAddress}), "Not dao");
      });
    });
  });
});